...
// Draw a single prompt: icon and label. For double-row slots pass iconRowVY != textRowVY (icon on row 0, text on row 1).
// For single-row (popups) pass the same Y for both. Use with layout slot macros.
...
void uiVirtualRectToNative(int vx1, int vy1, int vx2, int vy2,
