...
// The legacy IOMAN fallback still needs its internal device-table offset.
// Modern IOMANX exposes its table through iomanX_GetDeviceList(); using that
// public export is important because the private module layout has changed
// since this server was originally written.
...
iop_device_t *FileSystem_ScanDevice(const char *pDevice, int iNumDevices, const char *pPath)
{
    ...
    if (!strcmp(IOPMGR_IOMANX_IDENT, pDevice)) {
        ppkDevices = iomanX_GetDeviceList();
    } else if (!strcmp(IOPMGR_IOMAN_IDENT, pDevice)) {
        const ModuleInfo_t *pkModule = FileSystem_GetModule(pDevice);
        if (NULL == pkModule)
            return NULL;
        ppkDevices = DEVINFOARRAY(pkModule, DEVINFO_IOMAN_OFFSET);
    } else {
