/*
  ...
  Vendored from Open PS2 Loader commit
  3e3f34e9f94b058f7fd5b13727cb86e94fd3b35d. NHDDL executes this one-shot
  module only after a fresh IOP reset and after its recovery callback is armed.
*/
...
int _start(int argc, char **argv)
{
    ...
    /* Let the SPU2 assert any interrupt that it needed to assert here.
      Otherwise, the IOP may crash when OSDSYS loads CLEARSPU, as its interrupt
      handler jumps to a NULL pointer. For reasons, this doesn't seem to work
      when interrupts are enabled before the SPU reset. */
