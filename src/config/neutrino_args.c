...
int loadTitleNeutrinoArguments(ArgumentList *result, Target *target) {
  ...
  DPRINTF("config: looking for title-specific config for %s (%s)\n", target->name, target->id);
  ...
  if (targetPath[0] == '\0') {
    DPRINTF("config: title-specific config not found\n");
    return 0;
  }

  // Load arguments
  DPRINTF("config: loading title-specific config from %s\n", targetPath);
  int ret = loadArgumentList(result, device, targetPath);
  if (ret) {
    DPRINTF("config: error: Failed to load argument list: %d\n", ret);
  }
...
int saveTitleNeutrinoArguments(Target *target, ArgumentList *options) {
  ...
  DPRINTF("config: saving title-specific config to %s\n", lineBuffer);

  // Open file, truncating it
  int fd = open(lineBuffer, O_WRONLY | O_CREAT | O_TRUNC);
  if (fd < 0) {
    DPRINTF("config: error: failed to open file\n");
    return fd;
  }
  ...
  while (tArg != NULL) {
    ...
    if (len > 0) {
      if ((ret = write(fd, lineBuffer, len)) != len) {
        DPRINTF("config: error: failed to write to file\n");
        goto out;
      }
    }
    tArg = tArg->next;
...
int saveGlobalNeutrinoArguments(struct BackendDevice *device, ArgumentList *options) {
  ...
  if (stat(targetPath, &st) == -1) {
    DPRINTF("config: creating config directory: %s\n", targetPath);
    mkdir(targetPath, 0777);
  }

  buildConfigFilePath(targetPath, device->mountpoint, globalOptionsPath);
  DPRINTF("config: saving global config to %s\n", targetPath);
